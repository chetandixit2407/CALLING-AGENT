...
export interface GeminiLiveCallbacks {
  ...
  onWsClose?: (event?: any) => void;
  onAudioChunkSent?: (chunkCount: number, byteLength: number) => void;
}
...
export class GeminiLiveClient {
    ...
    try {
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          console.log(`[GeminiLiveClient:${this.sessionId}] [WebSocket Lifecycle] 'onmessage' event received. Type="${msg.type || 'unknown'}", Keys=[${Object.keys(msg).join(', ')}], PayloadSize=${event.data?.length || 0} bytes`);
          this.callbacks.onWsMessage?.(msg);
      ...
      ws.onerror = (err) => {
        ...
        console.error(`[GeminiLiveClient:${this.sessionId}] [WebSocket Lifecycle] 'onerror' event caught:`, err);
        this.callbacks.onWsError?.(err);
      ...
      ws.onclose = (event: CloseEvent) => {
        ...
        console.log(`[GeminiLiveClient:${this.sessionId}] [WebSocket Lifecycle] 'onclose' event triggered. Code=${event?.code}, Reason="${event?.reason || 'none'}", wasClean=${event?.wasClean}`);
        this.callbacks.onWsClose?.(event);
  ...
  public async startCall(
    ...
    return this.start(candidate, scenario, availableSlots, languagePreference);
  }
  ...
  private setupMicProcessor(): void {
    ...
    this.processorNode.onaudioprocess = (e) => {
      ...
      if (this.audioChunksSent % 50 === 0) {
        console.log(`[GeminiLiveClient:${this.sessionId}] [Microphone Stream] Sent 50th audio chunk (#${this.audioChunksSent}, size=${base64Audio.length} chars) to Gemini WebSocket`);
        this.callbacks.onAudioChunkSent?.(this.audioChunksSent, base64Audio.length);
      }
    };
  }
